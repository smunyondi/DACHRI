  ...
  return (
    ...
    >
      ...
      <div className="absolute left-4 bottom-4 flex flex-col justify-end w-2/3 bg-white bg-opacity-80 rounded p-2 group-hover:bg-opacity-100 group-hover:shadow-lg transition-all duration-300">
        ...
        <p className="text-lg font-semibold text-dachriRed group-hover:text-dachriNavy transition-colors duration-300">${product.price}</p>
      </div>
      <button
      ...
      <AuthModal isOpen={authOpen && !localStorage.getItem("token")} onClose={handleAuthClose} />
    </div>
  );
};
